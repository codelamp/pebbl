async('theory.plugins.events', ['theory'], function(theory){
  ...
  theory.plugins.string = (function(mixin){
    ...
    var local = theory.base.mix(mixin || {}, {
       ...
       * @namespace theory.string.shared
       */
      shared: {
        /**
      ...
      parse: function(name, instructions){
        if ( instructions || (instructions=this.shared.instructions[name]) ) {
      ...
      instructions: t.method({
        ...
        method: function(name, instructions){
          if ( arguments.length > 1 ) {
            this.shared.instructions[name] = instructions;
            return this;
          }
          else {
            return this.shared.instructions[name];
          }
        },
        attributes: {
          /**
              ...
              if ( mode.content ) {
                ...
                if ( snowball.children ) {
                  for ( a=snowball.children, i=0, l=a.length; i<l; i++ ) {
                    this.instructions.processGroup( instructions, name, a[i] );
                  }
                }
                else {
                  for ( a=mode.content, i=0, l=a.length; i<l; i++ ) {
                    this.instructions.processItem( instructions, a[i], snowball );
                  }
                }
              }
            }
