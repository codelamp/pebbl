async('theory.plugins.navigate', ['theory', 'theory.is'], function(theory, is){
   ...
   * @namespace
   */
  theory.plugins.navigate = (function(mixin){
    ...
    var local = theory.base.mix(mixin || {}, {
       ...
       *
       * @memberof! theory.navigate
       * @namespace
       * @prop {RegExp} simpleSelector - the expression to detect simple selectors i.e. `"attribute"`
       * @prop {RegExp} lispSelector - the expression to detect the start of lisp-like selector patterns i.e. `"($command"`
       * @prop {RegExp} segmentedSelector - the expression to detect if a selector contains segments i.e. `"attribute/attribute"`
       * @prop {RegExp} endsWithSelector - the expression to detect if a selector contains an ends with selector i.e. `"attr$"`
       * @prop {RegExp} startsWithSelector - the expression to detect if a selector contains an starts with selector i.e. `"^attr"`
       * @prop {RegExp} empty
       */
      shared: {
      ...
      configuration: function(config){
        if ( config ) {
          config.hasChildren    && (this.i.hasChildren    = config.hasChildren);
          config.children       && (this.i.children       = config.children);
          config.hasParent      && (this.i.hasParent      = config.hasParent);
          config.parent         && (this.i.parent         = config.parent);
          config.needsResolving && (this.i.needsResolving = config.needsResolving);
          config.resolve        && (this.i.resolve        = config.resolve);
        }
        return this;
      },
